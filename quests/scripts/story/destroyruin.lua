...
function sendMessageWithPlayerTagByNebulox(targetEntityId, messageString, playerName, portraitImage)
 local newString = messageString:gsub("<entityName>", playerName)
  
  world.sendEntityMessage(targetEntityId, "queueRadioMessage", {
      messageId = "destroyruin_angela",
      unique = false,
      important = true,
      type = "mission",
      text = newString,
      ...
      portraitFrames = 4,
      persistTime = nil,
      chatterSound = "/sfx/interface/angelachatter_slow_loop.ogg"
    },
  0)
end
...
local playerName = world.entityName(playerId)
 sendMessageWithPlayerTagByNebulox(playerId, "Congratulations on saving the Universe. For a moment I've lost your life readings, but in the end you did a good job, ^#B0C4DE;<entityName>^white;. You've grown as Protector so much, it's truly commendable.", playerName, "/ai/portraits/angela_portrait.png:talk.<frame>")
  end
